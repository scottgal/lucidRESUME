...
public static partial class SectionClassifier
{
    ...
    [
        (["experience", "employment", "work", "work history", "career history", "professional history", "professional background", "positions held"], "Experience"),
    ...
    public static string? ClassifyHeading(string heading)
    {
        ...
        foreach (var (keys, section) in SectionMap)
        {
            ...
            {
                if (clean.Equals(k, StringComparison.Ordinal) ||
                    clean.StartsWith(k + " ", StringComparison.Ordinal))
                    return section;
